...
FAQ_RULES = [
    ...
    (
        ...
        [
            ...
            "چاتجيبيتي", "جاتي", "بالجات", "بالشات", "بالچات",
            "للجات", "للشات",
        ],
    ...
    (
        "amboss",
        ["امبوس", "انبوس", "ام بوس", "ان بوس", "ambos", "amboss"],
        "متوفر عندنا Amboss ثلاثة أشهر، حساب خاص بـ20 ألف",
    ),
    (
        "anki",
        ["انكي", "anki", "آنكي", "انچي"],
    ...
    (
        ...
        [
            ...
            "التلي المميز", "تلي المميز", "تلي مميز", "تلغرام مميز", "telegram premium", "premium",
        ],
        "متوفر تلث اشهر ب 25 اما السنة ب 35 والسنة ب55 الف",
    ),
]
...
def _dedupe_campaign_recipients(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        try:
            ...
            continue
        # Telegram private-user chat IDs are positive. Negative IDs here are
        # generated by the owner's interactive test mode (or groups/channels),
        # so they must never enter a customer campaign.
        if chat_id <= 0:
            continue
...
async def process_campaign_queue(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    recipient_chat_id = int(recipient["customer_chat_id"])
    if recipient_chat_id <= 0:
        # Older campaigns may already contain synthetic interactive-test rows.
        # Skip them quickly so a running campaign can reach real customers.
        try:
            attempted_count = int(campaign.get("attempted_count") or 0) + 1
            supabase.table("customer_campaign_recipients").update({
                "delivery_status": "skipped",
                "error_text": "Synthetic or non-private Telegram chat id",
            }).eq("id", recipient["id"]).execute()
            supabase.table("customer_campaigns").update({
                "status": "sending",
                "attempted_count": attempted_count,
                "skipped_count": int(campaign.get("skipped_count") or 0) + 1,
                "next_send_at": (now + timedelta(seconds=1)).isoformat(),
            }).eq("id", campaign["id"]).in_("status", ["queued", "sending"]).execute()
        except Exception:
            logger.exception("Failed to skip invalid campaign recipient %s", recipient_chat_id)
        return
    ...
    try:
        ...
        await send_personal_message(
            recipient_chat_id, campaign["message_text"],
